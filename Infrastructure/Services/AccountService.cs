...
public class AccountService(UserManager<UserEntity> userManager, UserRepo userRepo, IConfiguration configuration)
{
    ...
    public async Task<User> GetUserAsync(ClaimsPrincipal claimsPrincipal)
    {
        ...
        var userEntity = await _userRepo.GetAll().Include(x => x.Address).FirstOrDefaultAsync(x => x.Id == nameIdentifer);
        if (userEntity != null)
        {
    ...
    public async Task<bool> UpdateBasicInfoAsync(ClaimsPrincipal claimsPrincipal, AccountBasicInfo basicInfo)
    {
        ...
        {
            var nameIdentifer = claimsPrincipal.FindFirst(ClaimTypes.NameIdentifier)!.Value;
            var userEntity = await _userRepo.GetAsync(x => x.Id == nameIdentifer);
            if (userEntity != null)
            {
    ...
    public async Task<bool> UpdateAddressInfoAsync(ClaimsPrincipal claimsPrincipal, AccountAddressInfo addressInfo)
    {
        ...
        {
            var nameIdentifer = claimsPrincipal.FindFirst(ClaimTypes.NameIdentifier)!.Value;
            var userEntity = await _userRepo.GetAll().Include(i => i.Address).FirstOrDefaultAsync(x => x.Id == nameIdentifer);
